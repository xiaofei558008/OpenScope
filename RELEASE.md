...
4. 提交 git + 打 tag + 推送双远端（gitee/github）：`git tag v1.9.0`，见 PROGRESS.md 惯例
...
└── openscope/
    └── OpenScope-Setup-1.9.0.exe   # 各版本安装包
```

nginx：`location ^~ /downloads/` → alias `/var/www/downloads/`，.exe 返回
`Content-Disposition: attachment; filename="OpenScope-Setup-1.9.0.exe"`（**必须带 filename=**，
...
- 可手工核验 SHA256：下载页每行展示，或本地 `python -c "import hashlib;print(hashlib.sha256(open(r'dist\OpenScope-Setup-1.9.0.exe','rb').read()).hexdigest())"`
