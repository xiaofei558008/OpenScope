...
- **checkpoint-55（2026-08-23）**：上传/下载 ELF 变量列表真正应用（用户要求：远端软件变量列表用对端解析结果更新，列表展示同步刷新）（1.22.2）。
  - **上传ELF**：把本机解析后的变量列表（名称+地址+大小）广播给远端 → 远端**应用到自己的变量列表**；**下载ELF**：请求远端回发列表 → 本机应用。此前接收方只记日志不应用。
  - **应用语义**：新增叶变量（无本地 ELF 也能观测——"编译 elf/out 可能是本地/网络端产生"场景闭环）；同名变量**更新地址**（对端编译产物地址不同）；无 DWARF 类型按大小给默认类型（1/2/4/8→有符号整型）。合成变量持久在 g_app.synth_*（256 上限），ELF 重载后自动重新应用；本机无 ELF 时树照常显示同步变量（is_synth 叶追加展示）。
  - **树展示同步刷新**：leaf_add 批量应用完成后投递 WM_OS_LEAF_SYNC → UI 线程重建变量树 + 补挂布局待绑定变量 + 状态栏刷新，日志"变量树已刷新: 叶表 N 个变量"。
  - 框架 v4 扩展回调：leaf_add（1=新增/2=更新地址）、leaf_sync_done、leaf_size（变量表上传用叶定义大小——此前取样本 size，未采样过的叶大小=0）。
  - 测试：net_drive.ps1 升级 25 项——**B/C 不再加载本地 ELF**，全靠"下载ELF"应用 A 的 4 变量列表后勾选/建窗/监视/显示（断言"网络变量同步: 新增 4 个"、"变量树已刷新: 叶表 4"）；A 收到 B 应用后二次上传的 4 项列表（双向核对）。连续 3 次 ALL PASS；安装版 net_drive/net_ui ALL PASS；ui_windows/features/layout/n9_watch ALL PASS。
  - 版本 1.22.2：重新打包 `dist/OpenScope-Setup-1.22.2.exe`，静默安装验证 1.22.2.0。
- **checkpoint-54（2026-08-23）**：修复网络采集失败（用户实测报障：远端下达监视后探针侧"采集停止：MCU 连接已断开"、0 样本、日志风暴）（1.22.1）。
  - **根因 1（数据拿不到）**：远端监视重建会**清掉探针本地勾选**——旧实现按"一次性快照∪远端引用"重建全局监视集，快照时机不对时把 11 个本地勾选叶覆盖为 0（观测数 11→1），探针采集被打断/降级。
  - **根因 2（"采集停止：MCU 连接已断开"）**：无硬件连接的实例（远端也按了监听形成双向连接时）收到对方监视列表也会 rebuild→acq_start，启动注定失败的采集线程。os_ds_start 新增 IS_CONNECTED 快速失败（"MCU 未连接，无法开始采集"），不再启动死线程。
  - **根因 3（日志风暴）**：每次 WATCH_LIST 全量重建 + 每叶 set_watch 必记日志。改为：仅状态翻转时勾选/记日志；重复监视列表为纯 no-op。
  - **监视集语义重构**：每叶引用计数 + 首被远端监视时记住本地状态；最后一个远端引用归零时**还原到远端监视前状态**（不覆盖本地勾选、不留僵尸监视）；停止下达只在实际有远端监视时停采，本机仍有勾选则继续采集并明示日志。
  - 测试增强（net_drive.ps1 22 项）：A 本地勾选 g_counter + B 下达 2 项 → A 采集并集=2 叶；B/C 停止下达（新增 --net-watchstop=毫秒 钩子）→ A 保留本地勾选继续采集（1 叶速率日志持续）。连续 3 次 ALL PASS；安装版 net_drive/net_ui/connect 全部 ALL PASS。
  - 版本 1.22.1：重新打包 `dist/OpenScope-Setup-1.22.1.exe`，静默安装验证 1.22.1.0。
- **checkpoint-53（2026-08-23）**：需求14 网络配置 UI 整合（用户要求：网络配置与 J-Link/ST-Link 配置合并为一栏，通道下拉含网络选项，按键复用，新增网络菜单）（1.22.0）。
  - **单行工具栏**：原第二行网络配置行并入首行（IP/端口编辑框 + 监听/上传ELF/下载ELF 按键），两行工具栏恢复为一行（bh 62→34，波形/数值/消息区域纵向空间增大）。
  - **通道下拉含"网络"选项**：IDC_CFG_DRIVER（J-Link/ST-Link）末项新增"网络"；选中后接口/速度/设备/刷新等硬件配置控件自动禁用（net_mode_apply_controls），状态栏显示"通道: 网络"。
  - **按键复用**：通道=网络时，连接=网络连接、断开=网络停止、开始采集=下达监视列表（OS_CMD_NET_WATCH）、停止采集=停止下达（新增 OS_CMD_NET_WATCH_STOP=29：发空监视列表，探针侧撤销该客户端监视并在无远端监视时自动停采；WATCH_LIST 处理改为按客户端引用计数重建全局监视集=本地勾选∪各远端并集，本地勾选快照保留）。
  - **网络菜单**（菜单栏"网络(&N)"）：网络配置…（新增 OSDlgNetCfg 对话框：IP/端口编辑+确定回写工具栏内联编辑框）、监听/连接/断开、同步采集/停止下达/下载记录、上传/下载 ELF——与工具栏同源。
  - **修复 2 个真 bug**：(1) 监听/上传ELF/下载ELF 按键被 g_tool_btns 循环与网络块重复创建（同 ID 两套子窗口）；(2) 网络配置对话框 OK 后读已释放状态（WM_DESTROY free + 对话框函数再读的 use-after-free）→ 状态改由对话框函数释放。
  - **测试**：新增 `tests/ui_net_ui_drive.ps1`（13 项断言：单行布局 y 差、通道下拉含网络、菜单含网络、网络模式下硬件控件禁用、配置对话框初值/回写+日志、菜单监听、B 实例按键复用连接/开始采集/停止采集/断开全链路），接入 build_tests；net_drive.ps1 UI 断言同步更新（监听/上传ELF/下载ELF 按键 + IP/端口编辑框）。开发版+安装版全部 ALL PASS；ui_windows/features/layout/theme_dark/n9_watch/connect(真机) 全部 ALL PASS。
  - 测试钩子新增 `--net-set=ip:port`（进程内设置内联网络配置——跨进程 WM_SETTEXT 对 EDIT 不可靠，测试用它预置）；测试侧 EDIT 实时文本改用 WM_GETTEXT（跨进程 GetWindowTextW 对 EDIT 只返回创建时文本）。
  - 版本 1.22.0：重新打包 `dist/OpenScope-Setup-1.22.0.exe`（14.2MB），静默安装验证 1.22.0.0 + 网络 UI 全套功能。
