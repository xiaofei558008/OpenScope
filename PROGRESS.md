...
- **checkpoint-29（2026-08-10）**：Rust 2.0.0 exe 安装包打包完成 + 发布（用户："我没看到exe 安装包打包完成"）。
  - 新增 `packaging/openscope-rust.iss`（Inno Setup，AppVersion 2.0.0.0 / OutputBaseFilename OpenScope-Setup-2.0.0）：安装 `target/release/openscope-app.exe` → `{app}\OpenScope.exe` + `dll\JLink_x64.dll`；[InstallDelete]/[UninstallDelete] 清理旧 C 版 dll/ 模块残留。
  - 修复两个打包回归：`main.rs` arg parsing 丢失 `selftest=true`（--selftest 不再跑自测）→ 恢复单独分支；release 目录缺 JLink_x64.dll → 复制后 release selftest exit=0（4833 samples/var）。
...
- **checkpoint-31（2026-08-10）**：修复 request.md "rust开发 bug" 3-6 —— ELF 只解析全局变量 + 菜单窗口可添加波形/数值窗口 + 变量树右键菜单 + 数值窗口，v2.0.2。
  - **bug3（ELF 解析出函数名）根因**：`openscope-elf/src/lib.rs` 收集全部具名定义符号（含 STT_FUNC main），未按符号类型过滤 → 树中出现函数名。修复：仅保留 `SymbolKind::Data/Tls/Common` + `is_global()` + `size()!=0`，剔除函数与未知段；`elf_sample.out` 由 5 符号过滤为 3 个全局变量（g_counter/g_cfg/g_raw），`ui_rust_elf_drive.ps1` C2 改为期望 3 项。
  - **bug4（菜单窗口置灰无法添加）根因**：菜单"窗口"项未接 add_window。修复：`app.rs` 点亮 IDM_WIN_CHART/IDM_WIN_NUM（去 MF_GRAYED）并在 WM_COMMAND 接 `add_window(WinKind)`；新增 `winmgr.rs`（原 windows.rs，规避 Rust 2018 `mod windows` 遮蔽外部 windows crate）——每窗口=一个 tab（TabControl），`OsWin`{kind,hwnd,title,series}，TCN_SELCHANGE 切换显示，新增 tab 自动选中并铺满显示区。
  - **bug5（ELF 树右键无菜单）根因**：树 WM_CONTEXTMENU 未实现。修复：`show_tree_context_menu`（弹 添加变量到波形/数值窗口 + 全部勾选/全部取消勾选）；`checked_leaf_idxs` 走 TVM_GETNEXTITEM 遍历勾选项；`ctx_add_checked_to` 目标窗口优先当前 tab（类型匹配）→ 该类型最近 → 自动新建；`ctx_check_all` 用 TVM_SETITEMW 状态图 2/1 同步。图例：`tabs/ui_rust_windows.ps1` D 段实测弹出菜单 5 项（含分隔符）文本正确（经 MN_GETHMENU 读取）。
  - **bug6（数值窗口未实现）**：新增 `numwin.rs`（OpenScopeNum 类）——ListView 4 列（实时☑/变量名/值/地址，LVS_EX_CHECKBOXES），顶部编辑框+写入按钮，`do_write` 经 `jlink.write_mem` 就地写入（浮点/hex/十进制解析，按 size 截断小端），勾选列控制该行实时刷新。`chart.rs` 增强：每窗口独立系列（窗口 ID 反查 winmgr）、点数少画圆点、滚轮缩放、右键移除系列。windows-rs 0.61 签名兼容：`TrackPopupMenu(nreserved=None)`、`LVITEMW.iItem/isubItem` 为 i32、`LIST_VIEW_ITEM_STATE_FLAGS`、`RedrawWindow(Some(hwnd))` 等。
  - 回归（源构建 + 已安装 exe 三回归均 ALL PASS）：`tests/ui_rust_elf_drive.ps1`（C2=3 项验证 bug3）、`tests/ui_rust_menu.ps1`（菜单回归）、新增 `tests/ui_rust_windows.ps1`（A 菜单不置灰/bug4；B/C 菜单添加窗口+tab 映射 ID；D 右键菜单弹出+5 项/bug5；E 全部勾选+自动建数值窗口显示 3 变量/bug6）。**跨进程坑**：SendMessage 传外进程结构体指针（LVM_GETITEMTEXTW/TCM_GETITEMW 文本读取）会让目标进程崩溃——测试只做标量级断言（计数/控件 ID/窗口类/菜单状态）。
  - 版本 2.0.2（`Cargo.toml` workspace + `app.rs VERSION` + `packaging/openscope-rust.iss` AppVersion 2.0.2.0）；ISCC 编译 `dist/OpenScope-Setup-2.0.2.exe`（10.4MB）；`/VERYSILENT` 安装（复用已注册 AppDir=`rust/install_test`）后已安装 exe 三回归 ALL PASS。
