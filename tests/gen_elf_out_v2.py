"""Generate tests/elf_sample_v2.out: elf_sample.out 的"重新编译"变体（需求2 重绑回归）。

与 elf_sample.out 的差异（模拟用户重新编译后变量顺序漂移 + 地址变化）：
  - 新增 g_extra : int @ 0x20000008，排在 DWARF 变量首位（叶下标整体后移）
  - g_counter : 移到 @ 0x20000004（原 0x20000000）
  - g_cfg     : 移到 @ 0x20000018（原 0x20000010）
  - g_raw     : 保持 @ 0x20000020（symtab only）

旧实现窗口只存叶下标：重建后下标指向不同变量 => 绑错变量（静默错地址）。
新实现按变量全名重绑 => g_counter 应重绑到新下标并显示新地址 0x20000004。

用于 tests/ui_elf_rebind_drive.ps1：加载 elf_sample.out 添加 g_counter 后，
用本文件覆盖原路径触发"ELF 更新"弹窗，确认后验证重绑日志与新地址。
"""
import struct
...
body.extend(die(1, b"cu_v2.out\x00" + struct.pack("<H", 0x1C)))  # DW_LANG_C99
...
# 变量顺序：g_extra 在前（下标漂移）→ g_counter → g_cfg
body.extend(die(5, b"g_extra\x00" + struct.pack("<I", int_off) +
                 b"\x05\x03" + struct.pack("<I", 0x20000008)))    # DW_OP_addr
body.extend(die(5, b"g_counter\x00" + struct.pack("<I", int_off) +
                 b"\x05\x03" + struct.pack("<I", 0x20000004)))    # 新地址
body.extend(die(5, b"g_cfg\x00" + struct.pack("<I", cfg_off) +
                 b"\x05\x03" + struct.pack("<I", 0x20000018)))    # 新地址
...
# ---------------- symtab / strtab ----------------
strtab = b"\x00g_extra\x00g_counter\x00g_cfg\x00g_raw\x00main\x00"
off_g_extra = strtab.index(b"g_extra")
...
for name, val, size, info_byte in [
    (off_g_extra,   0x20000008, 4, 0x11),  # STB_GLOBAL | STT_OBJECT
    (off_g_counter, 0x20000004, 4, 0x11),
    (off_g_cfg,     0x20000018, 8, 0x11),
...
Path("tests/elf_sample_v2.out").write_bytes(bytes(out))
print("wrote tests/elf_sample_v2.out (%d bytes)" % len(out))
